...
/**
 * Application service â€” scans all transfers in {@code PENDING_APPROVAL} and expires
 ...
 *   <li>Domain enforces the {@code PENDING_APPROVAL â†’ EXPIRED} transition.</li>
...
public class ExpirePendingTransfersService implements ExpirePendingTransfersUseCase {
    ...
    private static final String EXPIRATION_REASON =
            "Vencida por falta de aprobaciÃ³n en el tiempo establecido";
    ...
    public List<Long> expirePendingTransfers(LocalDateTime now, Duration expirationWindow) {
        ...
        for (Transfer transfer : pending) {
            ...
            // 3. Apply domain state transition â€” translate IllegalStateException so the
            ...
            // 5. Audit â€” include the reason mandated by the PDF spec
            auditLogPort.save(
                    "Transfer",
